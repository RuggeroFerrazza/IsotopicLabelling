...
The purpose of this document is to explain how to use the *IsotopicLabeling* R package to analyse mass spectrometric isotopic patterns obtained following isotopic labeling experiments.

A typical labeling experiment makes use of substrates enriched in one stable isotope, such as ^2^H or ^13^C; consequently, after the growth period, there are expected to be metabolites that will have incorporated the labeling isotope, and therefore its relative distribution within those metabolites will be different from its natural occurrence. The *IsotopicLabeling* R package is based on the principle that, since the isotopic patterns obtained in mass spectrometry reflect the isotopic compositions of the elements making up the observed species, the amount of labeling can be assessed by their proper examination. 
...
Below is a step-by-step explanation on how to use the *IsotopicLabeling* package; the example data set that comes with the package will be used, and the involved functions will be properly introduced and discussed.

# The MS Data Set

The *IsotopicLabeling* package requires the MS data to be a data frame with the first two columns representing *m/z* and retention time (RT) of the mass peaks, and the other columns containing peak intensities or areas (one column for each sample to be analysed).

Since a popular R package for handling MS data is *xcms*, the *IsotopicLabeling* R package can also read in *xcmsSet* objects, which basically contain the peak intensities or areas associated to each of the processed samples, together with their average retention time and *m/z*. In this case, the function `table_xcms` is available for converting such objects to the required data frame.
...
This is an *xcmsSet* object representing lipid extracts of 8 samples from ^13^C labeling experiments:

- The first 4 samples are relative to unlabeled cell cultures (natural ^13^C abundance);
...
- **initial_abundance**, either NA (the default value) or a numeric vector with length equal to the number of samples, containing the initial estimated percentage isotopic abundances of the labeling isotope. If provided, numbers between 0 and 100.
...
Further details will be given in the following sections, where each of the steps undertaken by `main_labeling` will be discussed and critically discussed.

# Step-by-Step Processing
 
The *IsotopicLabeling* package aims to find the abundance of the isotope used during the labeling experiment within the compounds of interest, based on their MS isotopic patterns. A number of algorithms are available to a-priori compute the isotopic patterns knowing the isotopic abundances; the goal of the *IsotopicLabeling* package is the opposite: starting from measured experimental patterns, this package aims at understanding which is the isotopic distribution giving rise to those patterns. This is basically achieved by a fitting procedure through which the abundance of the labeling isotope is iteratively changed until the best match between predicted and experimental patterns is found.

The `main_labeling` function basically performs three successive steps:

1. It gathers information about the isotopes and all the possible isotopologues arising from the labeling (`isotopic_information` function);
2. It extracts from the experimental data the isotopic patterns of the compound of interest (`isotopic_pattern` function);
3. It analyses the extracted patterns to estimate the percentage isotopic abundance of the labeling isotope.

An alternative to using `main_labeling` is to run the three distinct functions it is made of, and this route is covered below.

## Isotopes and Isotopologues

The first function used by `main_labeling` is `isotopic_information`, which summarizes important isotopic information in a single object, a list required by the subsequent functions. The input parameters are the chemical formula and the type of labeling; for [PC 32:2 + H]^+^, the list can be obtained through:
```{r, eval=FALSE}
...
attributes(info)
```
In particular, "isotopes" is a table with the natural isotopic abundances (numbers between 0 and 1) of the elements present in the compound. The two isotopes of the labeling element X are given NA values:
```{r}
info$isotopes
```

Importantly, "target" is a named vector with the exact masses of all the possible isotopologues arising because of the labeling isotope; in the example, [PC 32:2 + H]^+^ has 40 carbon atoms, and therefore the possible isotopologues coming from ^13^C span a 41 mass range: the lightest one is the monoisotopic species (with 40 ^12^C atoms), whereas the heaviest is the species with 40 ^13^C atoms. However, the isotopic patterns also depend on the other elements, and therefore the list of target isotopologues is further extended by two *m/z* units, enough for small and medium-sized molecules such as lipids and metabolites.
...
Two of the patterns extracted for [PC 32:2 + H]^+^ are shown in Figure 1: the first (to the left) is relative to an unlabeled sample, whereas the second one (to the right) is relative to a labeled sample (99% ^13^C labeling). 

![An example showing two of the patterns extracted from the experimental data; to the left is an unlabeled sample, to the right a labeled sample (99% ^13^C). ](Figure_1.pdf) 

In this simple case, the difference is straightforward: in the labeled sample the most intense signal is shifted 40 mass units upwards with respect to the monoisotopic peak, indicating that the most abundant species is the one where all 40 carbon atoms have been replaced by the labeling isotope, ^13^C. 

## Isotopic Pattern Analysis

The extracted patterns are then analysed by the function `find_abundance`, which takes each of them and finds the best theoretical pattern that reproduces it by iteratively changing the relative abundance of the labeling isotope.
...
The output is a class *labeling* object, which is a list containing the results of the isotopic pattern analysis:
```{r, results = "hide", echo = FALSE, message = FALSE, warning = FALSE}
...
```
The estimated percentage abundances of the labeling isotope are in "best_estimate" (numbers between 0 and 100), the standard errors from the fitting procedure are in "std_error", whereas the percentage deviations between best fitted and experimental patterns are in "dev_percent". The *m/z* values are in "x_scale", while in "y_exp" are their normalised intensities and in "y_theor" are those of the best fitted theoretical patterns. The differences between the two are in "residuals".
...
1. The generic function `summary` allows to quickly glance at the estimated percentage abundances:
    ...
    In this example, the average value is 1.08% for unlabeled samples (close to the natural ^13^C abundance, 1.07%), whereas it is 98.94% for labeled samples.
...
2. The generic function `plot` can be used to produce three types of plots, depending on the parameter "type". By default (type="patterns") a series of plots is returned, one for each sample, showing the normalised experimental patterns superimposed to their fitted theoretical patterns:
    ...
    ![Graphical summary of the isotopic pattern analysis for an unlabeled (top) and a labeled (bottom) sample. ](Figure_2.pdf)
    ...
    ![Plot of the residuals for an unlabeled (top) and a labeled (bottom) sample. ](Figure_3.pdf)
...
3. The `save_labeling` function allows to save the results to a *.csv file in the working directory:
    ```{r, eval=FALSE}
...
In this document we introduced the main functions of the *IsotopicLabeling* R package, explaining their basic working principles. Using the provided data set, we illustrated how to use the package in practice and how to quickly assess the results, with the hope that this package will prove to be a useful tool to researchers dealing with labeling experiments.

After having discussed the principles behind this package, our final aim here is to provide a compact summary that may be used as a script for analysing LC-MS data relative to labeling experiments.
