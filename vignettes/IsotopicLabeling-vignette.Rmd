...
The output is an object of the class *labeling*, a list containing the results of the analysis:
```{r, results = "hide", echo = FALSE, message = FALSE, warning = FALSE}
...
Importantly, "target" is a vector with the exact masses of all the possible isotopologues coming from the labeling isotope. In the example, [PC 32:2 + H]^+^ has 40 carbon atoms, and therefore the possible isotopologues coming from ^13^C span a 41 mass range: from the monoisotopic species having 40 ^12^C atoms, to the species with 40 ^13^C atoms. In addition, since the isotopic patterns also depend on the other elements making up the compound, the list of target isotopologues is extended to two *m/z* units beyond the maximum achievable, which is enough for small and medium-sized molecules such as lipids and metabolites. 
